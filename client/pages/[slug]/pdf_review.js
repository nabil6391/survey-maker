...
export default function pdf_review(props) {
  ...
  const [dialogOpen, setDialogOpen] = useState(print != 'true')
  ...
  return (
    <Page >
            ...
            >
              <div className="bg-white inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform shadow-xl rounded-2xl">
                ...
                <div className="mb-6 pt-3 rounded flec flex-col">
                  ...
                  <a onClick={(e) => setDialogWait(true)} href={`/api/pdf?url=${encodeURIComponent(`${router.asPath}?DBar=${mDBar}&DPie=${mDPie}&mmBar=${mBar}&mmRadar=${mRadar}&Color=${mColor}&print=true`)}`} download="generated_pdf.pdf" className="block text-black" id=''>Generate</a>
                </div>
              </div>
            </Transition.Child>
          </div>
      ...
      <Transition appear show={dialogWait} as={Fragment}>
        <Dialog
          as="div"
          className="fixed inset-0 z-10 overflow-y-auto"
          onClose={(value) => setDialogWait(false)}
        >
          <div className="min-h-screen px-4 text-center">
            <Transition.Child
              as={Fragment}
            ...
            <Transition.Child
              ...
              leaveTo="opacity-0 scale-95"
            >
              <div className="bg-white inline-block max-h-md w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform shadow-xl rounded-2xl">

                Please wait for 2 mins to generate the pdf
              </div>
            </Transition.Child>
          </div>
        </Dialog >
